...
/**
 * Retrieve a list of native processes. Duplicates
 * org.netbeans.modules.java.lsp.server.debugging.attach.Processes Replace with
 * the direct use of ProcessHandle, after JDK 8 is abandoned.
 *
 * @author albilu
 */
final class Processes {
    ...
    static List<ProcessInfo> getAllProcesses() {
        ...
        Function<Stream<ProcessInfo>, List<ProcessInfo>> collector
                = infoStream -> infoStream
                        ...
                        .sorted((info1, info2) -> (info1.getPid() == info2.getPid() ? 0
                        : info1.getPid() > info2.getPid() ? -1 : 1)) // descending
    ...
    private static List<ProcessInfo> getProcesses(Class<?> processHandleClass,
            Function<Stream<ProcessInfo>, List<ProcessInfo>> collector) throws NoSuchMethodException,
    ...
    private static class ProcessHandle2Info implements Function<Object, ProcessInfo> {
        ...
        ProcessHandle2Info(Class<?> processHandleClass) throws NoSuchMethodException {
            pidMethod = processHandleClass.getMethod("pid");
            infoMethod = processHandleClass.getMethod("info");
            commandMethod = infoMethod.getReturnType().getMethod("command");
            commandLineMethod = infoMethod.getReturnType().getMethod("commandLine");
        }
